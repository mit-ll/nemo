...
void print_signals(vector<ivl_signal_t>& sigs){
	printf("\nNumber of Critical Signals Found: %lu\n", sigs.size());
	for (unsigned i = 0; i < sigs.size(); i++){
		print_full_signal_name(sigs[i]);
		print_signal_attrs(sigs[i]);
	}
	printf("\n");
}
...
int target_design(ivl_design_t des) {
	...
	ivl_net_const_t      con;           // temp constant object in design
	ivl_signal_t         aff_sig;       // temp signal object in the design
	...
	// Determine the dependencies for critical connection
	unsigned depth_counter = SEARCH_DEPTH;
