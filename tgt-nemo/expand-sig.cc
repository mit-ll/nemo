...
int expand_sig(ivl_signal_t aff_sig, Dot_File& df, set<ivl_signal_t>& sigs_to_expand, set<ivl_signal_t>& explored_sigs, bool expand_search) {
	// Device Pointers
	ivl_net_const_t con;
	...
	if (ivl_scope_is_cell(ivl_signal_scope(aff_sig)) && ivl_signal_port(aff_sig) == IVL_SIP_OUTPUT){
		depth_explored = expand_std_cell_sigs(aff_sig, df, sigs_to_expand, explored_sigs, expand_search);
	...
	for (unsigned int i = 0; i < ivl_nexus_ptrs(nex); i++) {
		...
		if ((sig = ivl_nexus_ptr_sig(nex_ptr))) {
			...
			if (ivl_logic_pin(logic, 0) == nex) {
				...
				depth_explored = expand_log(logic, aff_sig, df, sigs_to_expand, explored_sigs, expand_search);
			}
		}
			...
			if (ivl_lpm_q(lpm) == nex) {
				...
				depth_explored = expand_lpm(lpm, aff_sig, df, sigs_to_expand, explored_sigs, expand_search);
     		}
